...
def generate_ranges_by_granularity(total_pages: int, granularity: int) -> List[Tuple[int, int]]:
    """
...
def extract_pdf_pages(
    ...
    for start, end in ranges_iter:
        ...
        if output_path.exists() and not force:
            if quiet:
                print(f"Skipping existing file '{output_path}' (use --force to overwrite).", file=sys.stderr)
                continue
...
def extract_pages_as_png(
    ...
    """
    if fitz is None:
        print("Error: pymupdf required for PNG export. Install: pip install pymupdf", file=sys.stderr)
        return 0
    ...
    if not quiet and tqdm:
        pages_iter = tqdm(pages_list, desc=f"Exporting PNG from {input_path.name}", unit="page")
    else:
...
def extract_embedded_images(
    ...
    if fitz is None or Image is None:
        print("Error: pymupdf and Pillow required. Install: pip install pymupdf pillow", file=sys.stderr)
        return 0
    ...
    if not quiet and tqdm:
        pages_iter = tqdm(pages_to_process, desc=f"Extracting images from {input_path.name}", unit="page")
    else:
...
def get_pdf_info(input_path: Path, reader: Optional[PdfReader] = None) -> Optional[PdfReader]:
    """
...
def parse_size_spec(size_str: str) -> int:
    ...
    try:
        ...
        raise ValueError(f"Invalid size format: '{size_str}'. Use e.g., '10MB', '1GB', '500KB'")
    ...
    try:
        ...
        if reader.is_encrypted:
            ...
            try:
                ...
                if decrypted == 0:
                    print(f"Error: Could not decrypt '{input_path}'. Wrong password?", file=sys.stderr)
                    return False
            except Exception as e:
...
def merge_pdfs(
    ...
    try:
        ...
        for pdf_path in input_paths:
            ...
            if reader.is_encrypted:
                try:
                    ...
                    print(f"Warning: Could not decrypt '{pdf_path}', skipping.", file=sys.stderr)
                    continue
        ...
        if not quiet:
            print(f"  Merged {len(input_paths)} PDFs ({total_pages} pages) -> {output_path}")
        return True
...
def split_by_bookmarks(
    ...
    if not outlines:
        print(f"Warning: PDF '{input_path}' has no bookmarks. Using single output.", file=sys.stderr)
        ...
        for item in outline_items:
            if isinstance(item, list):
                ...
                try:
                    ...
                    title = item.title if hasattr(item, 'title') else f"Section_{len(bookmark_pages)+1}"
    ...
    def write_current_part() -> bool:
        ...
        try:
            ...
            if not quiet:
                ...
                print(f"  Created: {output_filename} ({len(current_pages)} pages, {size_kb:.1f} KB)")
            part_num += 1
            return True
        except Exception as e:
            print(f"Error writing '{output_path}': {e}", file=sys.stderr)
...
def optimize_pdf(
    ...
    """
    if fitz is None:
        print("Error: pymupdf required for optimization. Install: pip install pymupdf", file=sys.stderr)
        return (0, 0)
    ...
    try:
        ...
        if not quiet:
            ...
            print(f"  Optimized: {original_size / 1024:.1f} KB -> {new_size / 1024:.1f} KB ({reduction:.1f}% reduction)")
...
""",
    ...
    parser.add_argument(
        '--prefix',
        help='Custom prefix for output filenames (default: input filename)'
    )
    parser.add_argument(
        '--png',
        action='store_true',
    ...
    parser.add_argument(
        '--password',
        help='Password for encrypted PDFs (used with --unlock)'
    )
    parser.add_argument(
        '--by-bookmark',
        action='store_true',
    ...
    if args.merge:
        if not args.output:
            print("Error: --merge requires -o/--output to specify output file.", file=sys.stderr)
            sys.exit(1)
    ...
    if args.reverse:
        if not args.output:
            print("Error: --reverse requires -o/--output to specify output file.", file=sys.stderr)
            sys.exit(1)
        if len(pdf_files) != 1:
    ...
    if args.unlock:
        if not args.output:
            print("Error: --unlock requires -o/--output to specify output file.", file=sys.stderr)
            sys.exit(1)
        if len(pdf_files) != 1:
        ...
        success = unlock_pdf(pdf_files[0], output_path, args.password, args.force, args.quiet)
    ...
    if args.optimize and not args.pages and not args.png and not args.images and not args.by_bookmark and not args.max_size:
        if not args.output:
            print("Error: --optimize requires -o/--output to specify output file.", file=sys.stderr)
            sys.exit(1)
        if len(pdf_files) != 1:
    ...
    for pdf_file in pdf_files:
        ...
        if args.by_bookmark:
            count = split_by_bookmarks(pdf_file, output_dir, prefix, args.force, args.quiet)
            if not args.quiet:
        ...
        if max_size_bytes:
            count = split_by_size(pdf_file, output_dir, max_size_bytes, prefix, args.force, args.quiet)
            if not args.quiet:
        ...
        if args.images:
            count = extract_embedded_images(
                pdf_file, output_dir, ranges if args.pages else None,
                prefix, args.force, args.quiet
            )
            if not args.quiet:
                print(f"  Extracted {count} embedded image(s)")
        elif args.png:
            count = extract_pages_as_png(
                pdf_file, output_dir, ranges,
                prefix, args.dpi, args.force, args.quiet
            )
            if not args.quiet:
